...
# 保存為 PDF 和 TIFF 文件
output_dir = r"D:\Material_mask_autoencoder\results\pretrained_inclusion\i2c_finetune"
os.makedirs(output_dir, exist_ok=True)  # 確保目錄存在

# 定義不同 N_dataset 的數值
N_datasets = [5000, 1250]
...
for N_dataset, color in zip(N_datasets, colormap):
    ...
    for ratio in ratios:
        ...
        for comp in components:
            file_path = os.path.join(base_dir, f"i2c_{N_dataset}_finetune_inclusion_mr{ratio}_cls_{comp}/log.txt")
...
fig.legend(handles, labels, fontsize=6, frameon=True, loc='center left', bbox_to_anchor=(0.99, 0.5), 
           title="dataset size", title_fontsize=7, borderaxespad=0.5, labelspacing=0.5)
...
# 保存結果
plt.savefig(os.path.join(output_dir, "varies_dataset_i2c.pdf"), format="pdf", dpi=300, bbox_inches='tight')
plt.savefig(os.path.join(output_dir, "varies_dataset_i2c.tiff"), format="tiff", dpi=300, bbox_inches='tight')
plt.show()
